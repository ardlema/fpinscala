  ...
  object Process {
    ...
    def count[I]: Process[I,Int] = {
      def go(count: Int): Process[I,Int] =
        await(i => emit(count + 1, go(count + 1)))
      go(0)
    }
    ...
    def mean: Process[Double,Double] = {
      def go(elements: Int, sum: Double): Process[Double,Double] =
        await(i => emit((i + sum) / elements, go(elements + 1, sum + i)))
      go(1, 0)
    }
    ...
    def sumInTermsOfLoop: Process[Double,Double] =
      loop[Double, Double, Double](0.0)((d, acc) => (d+acc, d+acc))

    def countInTermsOfLoop[I]: Process[I,Int] =
      loop[Int, I, Int](0)((_, acc) => (acc + 1, acc + 1))
